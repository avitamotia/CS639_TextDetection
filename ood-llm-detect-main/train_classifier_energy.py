...
def train(opt):
    ...
    if opt.device_num>1:
        ...
        fabric = Fabric(accelerator="cuda", precision=opt.precision, devices=opt.device_num,strategy=ddp_strategy)#
    else:
        fabric = Fabric(accelerator="cuda", precision=opt.precision, devices=opt.device_num)
    fabric.launch()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--precision", type=str, default="16-mixed", help="Lightning Fabric precision (16-mixed for fp16, bf16-mixed for Ampere+)")
    parser.add_argument("--max_length", type=int, default=256, help="Max tokenizer sequence length; lower saves VRAM")
